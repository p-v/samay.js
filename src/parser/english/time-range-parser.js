...
export default class TimeRangeParser extends Parser {

  constructor() {
    super();
    this.regex = /(\b(?:(?:2[0-3])|(?:1\d)|(?:0?\d))(?:(?::|\s?)((?:0?\d)|(?:[0-5][0-9]?)))?\s{0,2}([ap](?:\.?m\.?)?)?\b)\s*(?:-|(?:to))\s*(\b(?:(?:2[0-3])|(?:1\d)|(?:0?\d))(?:(?::|\s?)((?:0?\d)|(?:[0-5][0-9]?)))?\s{0,2}([ap](?:\.?m\.?)?)?\b)/i;
  }
